...
ticket_sent_template = compiler.compile("\n".join((
    ...
    "",
    "В целях противодействия DDoS-атакам и обеспечения стабильной работы BMSTU Direct, мы можем временно ограничить возможность ответа на некоторые обращения. Однако хотим заверить вас, что все поступающие сообщения тщательно обрабатываются нашей командой.",
    "В ближайшее время мы планируем выпустить дайджест, в котором будут освещены наиболее актуальные вопросы и проблемы, затронутые в ваших обращениях. Благодарим за понимание и терпение!",
)))
